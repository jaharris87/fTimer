...
# MPI+OpenMP smoke build (compatibility plus opt-in ftimer_openmp worker API; no hybrid reductions)
...
- **`FTIMER_USE_OPENMP`** (CMake option, default OFF): Enables limited `!$omp master` guards around the guarded `ftimer_core` entry points and the explicit `ftimer_openmp_t` level-1 worker timing runtime. The existing `ftimer`/`ftimer_core` APIs remain master-thread-only, not full thread safety. The documented/supported build paths are GNU Fortran (`gfortran`) and LLVM Flang after CMake resolves `OpenMP::OpenMP_Fortran` and fTimer's configure-time OpenMP capability probe passes.
