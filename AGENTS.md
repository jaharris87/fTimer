...
- **Explicit OpenMP API surface**: `ftimer_openmp.F90` is the additive, OOP-first surface for future true worker timing. Its lifecycle/configuration and timer registration/lookup methods are real; otherwise valid timed-region and worker `start_id`/`stop_id` calls return `FTIMER_ERR_NOT_IMPLEMENTED` until the thread-lane runtime issues under #267 land, after lifecycle/context/id validation. The procedural default instance does not participate in true worker timing.
