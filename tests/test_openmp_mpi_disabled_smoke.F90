program ftimer_openmp_mpi_disabled_smoke
   ...
   call expect_status(ierr, FTIMER_ERR_NOT_INIT, 19)
   call expect_int(union_summary%num_ranks, 0, 20)
   call expect_int(union_summary%num_entries, 0, 21)

   call timer%print_mpi_openmp_union_summary(ierr=ierr)
   call expect_status(ierr, FTIMER_ERR_NOT_INIT, 22)

   call timer%write_mpi_openmp_union_summary(report_path, ierr=ierr)
   call expect_status(ierr, FTIMER_ERR_NOT_INIT, 23)
   call expect_file_absent(report_path, 24)

   call timer%write_mpi_openmp_union_summary_csv(csv_path, ierr=ierr)
   call expect_status(ierr, FTIMER_ERR_NOT_INIT, 25)
   call expect_file_absent(csv_path, 26)
