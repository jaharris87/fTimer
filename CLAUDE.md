...
- **Scoped guards are scalar ownership objects**: `ftimer_guard_t` balances a started timer on normal serial scope exit, but it must not be copied, assigned while active, placed in arrays, saved globally, or used as a function-return constructor. Guard finalizers diagnose mismatches to stderr and do not invoke warn/repair recovery.
