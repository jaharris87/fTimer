...
- Name-based `start`/`stop` remains the default ergonomic path. The runtime now uses an internal mapped lookup plus capacity-based growth so that default path scales better as the timer set grows.
