...
The public API supports these styles:
...
- Scoped guards start through the same validation and callback path as `start`/`start_id`. When an inactive guard's underlying start fails, the error is returned through `ierr` when present, or warned to stderr when `ierr` is omitted; the guard remains inactive and its finalizer is a no-op. Attempting to start an already active guard returns `FTIMER_ERR_ACTIVE` or warns, leaves the original activation armed, and does not replace it.
...
- `FTIMER_USE_OPENMP=ON` enables only limited master-thread-only guards. Worker-thread timer calls inside an OpenMP parallel region are silent no-ops. To time a parallel region as a whole, place `start`/`stop` or a scoped guard outside the `!$omp parallel` block. Do not rely on finalization of a block-local scoped guard inside a parallel region; if such a guard is started there deliberately, stop it explicitly on the master thread before leaving the region.
