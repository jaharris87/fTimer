...
For metadata headers, construct `ftimer_metadata_t` values by assigning `%key` and `%value` directly. These fields use allocatable-length storage, so assigned strings are not silently capped at the legacy 64-character threshold. Human-readable text reports escape metadata control characters, terminal escape bytes, backslashes, and leading blanks with the same visible policy used for formatted timer names. fTimer does not currently provide a helper constructor such as `ftimer_metadata(...)`; for formatted numeric metadata, write to a temporary character variable and then assign that string to `%value`.
...
- Human-readable local, strict MPI, and sparse MPI text reports escape metadata header keys and values before writing them. Tabs, newlines, carriage returns, delete, terminal escape bytes, other ASCII controls, backslashes, and leading blanks are rendered visibly instead of being emitted as raw log-control text; blank metadata values remain blank.
