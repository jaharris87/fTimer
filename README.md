...
- Human-readable local, strict MPI, and sparse MPI text reports escape metadata header keys and values before writing them. Tabs, newlines, carriage returns, delete, terminal escape bytes, other C0/C1 controls, backslashes, and leading blanks are rendered visibly instead of being emitted as raw log-control text; blank metadata values remain blank.
