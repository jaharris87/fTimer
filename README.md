...
- Explicit opt-in OpenMP API through `type(ftimer_openmp_t)` in `ftimer_openmp`, including keyword-required `init(config=...)`, optional MPI `comm=` capture in MPI builds, `reset`, `finalize`, `register_timer`, `lookup_timer`, `begin_parallel_region`, `end_parallel_region`, `start_id`, and `stop_id`
...
- `ftimer_openmp_t%init` requires `config=` and accepts `comm=` only by keyword in MPI builds. The OpenMP object keeps registered timer ids valid across `reset()` and invalidates them across `finalize()`/reinit without recycling ids in the same object. Worker-thread calls that omit `ierr` queue bounded diagnostics. A later serial lifecycle call without `ierr` emits one aggregate message when fTimer itself was built with `FTIMER_USE_OPENMP=ON`; with `ierr`, lifecycle calls that clear queued diagnostics return the first queued worker status without writing stderr. In non-OpenMP fTimer packages, `ftimer_openmp` is a serial-context lifecycle/catalog surface only, even if a downstream application separately enables OpenMP.
...
- `use ftimer_openmp` exposes the future opt-in worker-timing API shape without changing current `ftimer` behavior. The object lifecycle/configuration and timer catalog calls are available now; `begin_parallel_region`, `end_parallel_region`, `start_id`, and `stop_id` return `FTIMER_ERR_NOT_IMPLEMENTED` until the later OpenMP runtime issue implements thread-lane timing.
