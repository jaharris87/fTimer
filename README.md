...
- MPI call-count extrema remain exact `integer(int64)` fields. `avg_call_count` remains `real(wp)` and is computed without integer-sum overflow; for very large counts near the signed-64-bit limit, fTimer uses a conservative representable operand so the average does not round upward past the rank-local counts.
