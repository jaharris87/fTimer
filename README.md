...
- For `ftimer_openmp_t`, register names in serial context and pass ids into worker regions for the hot path. The OpenMP runtime keeps private catalog and lane-context indexes and grows segment storage only on participating lanes, so a public reserve/warm API is not currently required; use an untimed warm-up region when very short kernels should exclude first-touch allocation.
