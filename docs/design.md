...
**Scope (implemented through Phase 3):** A portable wall-clock timer library with profiling extensibility hooks plus local structured summaries and formatted reports. fTimer does NOT itself provide hardware counter or power measurements — it provides callback hooks so external tools (PAPI, likwid, etc.) can attach to timer boundaries in the future.
...
- pFUnit-backed behavioral tests exist for the Phase 2 core behaviors plus Phase 3 summary/self-time/file/callback coverage
- expanded procedural wrappers and MPI reductions are future implementation work
...
- **Current baseline**: buildable Phase 3 foundation + core runtime + local summary/reporting + smoke tests + opt-in serial pFUnit tests; later phases add procedural wrappers, MPI reductions, and OpenMP guards
