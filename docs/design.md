...
That means pFUnit-backed serial, OpenMPI, and OpenMP test jobs are part of current CI now; they are not deferred future work. The MPI smoke jobs use explicit MPI compiler and launcher paths, then exercise configure/build coverage, lifecycle-safe smoke checks, the MPI example smoke test, and installed-package consumer checks. The serial smoke executables and non-MPI example smokes are intentionally not run in MPI-enabled smoke jobs because they call fTimer outside the MPI lifetime; those runtime paths remain covered by the serial smoke job. The required MPICH job runs on `ubuntu-22.04`. `ubuntu-latest` currently maps to Ubuntu 24.04, whose MPICH package has an incorrect PMI/Hydra configuration on GitHub-hosted runners: two-process launches create two singleton `MPI_COMM_WORLD` instances of size 1. Keep the MPICH job pinned to a working hosted image until the distro package is fixed or CI installs a known-good MPICH build. MPICH pFUnit coverage is intentionally deferred from this first pass because the GitHub Actions trial generated pFUnit MPI tests that reported `PE=0` instead of launching with the requested 2- and 4-process counts. The contract-regression job also verifies the configure-time MPI/OpenMP gates and the documented Makefile wrapper behavior.
