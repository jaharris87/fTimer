...
- Local summaries expose context-cardinality diagnostics without changing timing behavior by default. `summary%total_contexts` is the total number of allocated parent-stack contexts across resident timers, `summary%max_contexts_per_timer` is the largest context count attached to any one timer name, and `summary%entries(i)%timer_context_count` repeats that per-timer count on each visible entry so callers can identify which timer name has high cardinality.
- Context-cardinality diagnostics count allocated runtime contexts, not just visible summary rows. A context can be allocated but hidden from the entry table after `reset()` or when it has no visible time/calls and is not active. These fields therefore diagnose runtime cardinality, while `summary%num_entries` remains the visible row count.
