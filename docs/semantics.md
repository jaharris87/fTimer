...
- Scoped timer guards are an optional procedural ergonomic layer over normal start/stop. `type(ftimer_scope_guard_t)` from `ftimer_core` or `ftimer` is initialized with `call ftimer_scope(guard, name, ierr=ierr)` for the procedural default instance. A successfully initialized guard performs a normal start event immediately and a normal stop event when `guard%close()` is called or when the guard is finalized at scope exit.
...
## Scoped Timer Guard Contract

- Scoped guards do not replace explicit `start`/`stop`; they are a convenience for lexical regions where a local object can own one balanced timer interval
- Guard initialization uses the same name validation, initialization checks, stack nesting, callback, and OpenMP master-thread-only behavior as normal `start`
- Guard finalization uses the same successful-stop accounting as normal `stop_id`: a successfully finalized guard fires the normal stop callback exactly once and attributes time to the guard's recorded parent context
- Guard close/finalization requires the guard-owned timer interval to be the exact top active interval in the recorded parent context. If another timer, another same-name context, or a later restarted interval for the same context is above it, `close(ierr=...)` returns `FTIMER_ERR_MISMATCH` and an active finalizer emits a stderr diagnostic. Scoped guards do not use warn/repair mismatch modes to infer which same-name context they own.
- Because Fortran final subroutines cannot return `ierr`, callers that need deterministic error handling should call `guard%close(ierr=ierr)` before leaving scope. If finalization has to stop the timer and that stop fails, the finalizer writes a diagnostic to stderr instead of silently swallowing the error.
- `guard%status()` returns the last start/close status recorded by that guard, and `guard%is_active()` reports whether the guard still owns an active timer context
- A failed explicit `guard%close(ierr=ierr)` leaves the guard active when the original timer context is still running, so a later correct close or the finalizer can still attempt to balance it. If another repair/warn operation already stopped that specific context, a later `guard%close(ierr=...)` is idempotent: it marks the guard inactive and returns `FTIMER_SUCCESS`.
- Assigning one scoped guard to another does not transfer or duplicate ownership. The assigned-to guard becomes inactive and records only the source guard's last status, so accidental copies cannot close the source guard's timer context.
- Finalizable OOP scoped guards for arbitrary `type(ftimer_t)` instances are deferred. The current accepted API points at the saved default instance behind `ftimer_scope(...)`, avoiding a non-portable stored pointer to a user timer object that might not have the `target` attribute.
- For nested scoped timers, prefer nested `block` constructs or separate procedure scopes so finalization order matches timer nesting. If several guard variables live in the same scoping unit, do not rely on compiler-specific finalization order to enforce strict timer nesting; call `close(ierr=...)` explicitly when order matters.
- The scoped API is intentionally subroutine-based rather than a function returning a guard object, avoiding reliance on compiler-specific function-result temporary finalization behavior.
