...
- CSV `append=.true.` appends records to the target file and omits the header when the existing file is non-empty. Non-empty append targets must begin with the fTimer CSV format-version-2 header, existing data rows must be complete fTimer v2 logical records with the exact v2 header field count and recognized `summary_kind`/`record_type` combinations, and the target must end with a newline; mismatched headers, older-format records, malformed v2-prefix records, or unterminated final records are rejected with `FTIMER_ERR_IO` instead of silently mixing schemas.
