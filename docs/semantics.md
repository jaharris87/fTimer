...
- Procedural users declare `type(ftimer_guard_t)` from `use ftimer` and start it with `ftimer_scope(guard, name, ierr)` or `ftimer_scope_id(guard, id, ierr)`. OOP users declare `type(ftimer_guard_t)` from `use ftimer_core` and start it with module procedures `ftimer_scope(timer_pointer, guard, name, ierr)` or `ftimer_scope_id(timer_pointer, guard, id, ierr)`.
...
- Starting a guard delegates to the same validation, stack push, call-count, clock, and callback behavior as the corresponding explicit start call. When an inactive guard's underlying start fails, the guard remains inactive; with `ierr` present the error is returned silently, and with `ierr` omitted the normal diagnostic path warns to stderr. Attempting to start an already active guard returns `FTIMER_ERR_ACTIVE` or warns, leaves the original activation armed, and does not replace it.
- On normal scope exit, an active guard finalizer stops the guarded activation only when that exact activation is still the current stack top. This stop is a normal user-visible stop event and fires the configured stop callback.
...
- Scoped guards are scalar ownership objects and must not be copied or assigned while active. fTimer diagnoses scalar assignment involving active guards and records an activation token so stale or copied guards cannot stop a later matching timer activation, but copied active guards can still create confusing diagnostics. Active guards in arrays are not supported; whole-array assignment and array finalization are not part of the portable scoped-guard contract.
- Scoped guards rely on standard Fortran finalization of local, non-`save` scalar variables on normal serial scope exit, including block exit and procedure return. They are not a cleanup mechanism for `stop`, `error stop`, process aborts, saved/module/global guard variables, guard arrays, block-local guard finalization inside OpenMP parallel regions, or compiler/toolchain finalization bugs.
