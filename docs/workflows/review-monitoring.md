...
4. Remember that the review router now reruns only the active Codex roles whose latest-push file delta still matches their routing rules on `synchronize`, reruns active labels on `ready_for_review`, and skips retriggering a role while its previous trigger comment still has Codex's `eyes` reaction, so inspect the latest trigger comment metadata before deciding a review is stale.
...
- If a role's previous trigger comment still has Codex's `eyes` reaction, the router now skips posting a new trigger for that role to avoid stacking overlapping reviews.
