...
4. Remember that the review router now reruns only the active Codex roles whose latest-push file delta still matches their routing rules on `synchronize`, and reruns active labels on `ready_for_review`, so inspect the latest trigger comment metadata before deciding a review is stale.
