...
4. Remember that the review router now reruns active Codex labels for the latest `head.sha` on `synchronize` and `ready_for_review`, so inspect the latest trigger comment metadata before deciding a review is stale.
...
- Trigger comments now include hidden `role`, `sha`, and prompt-version metadata so you can tell whether the latest commit has actually been queued for the expected review.
